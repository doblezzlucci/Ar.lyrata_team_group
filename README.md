...
All the scripts for downstream analysis available on the "src" folder  would be performed on  the 3 ouput files  of  GalaxyMasker tool available from https://usegalaxy.eu palteform which was used to process all  8 chromosome of A.Lyrata as well as A.lyrata toplevelel genome assembly (All lyrata)  and A.thaliana toplevelel genome assembly folders  (All thaliana) available on "file" folder 

 ### TE distribution 
 
The R notebook "TE_distribution.Rmd "  runs on RepeatMasker output file ( Galaxy_RepeatMasker...statitics.txt)  would generate TE distribution plots for families 

### Kimura plot

The R notebook "Kimura_plot.Rmd "  runs on RepeatMasker output file (Galaxy_RepeatMasker...catalogue.txt)  would generate Kimura plot divergences  for most abundant families of LTR (Gypsy and Copia) 

###  LTR insertion according to chromosome positions 

The R notebook "Inerstion_deltion_LTR_counts_plot__lyrata.Rmd "  run on RepeatMasker output file (Galaxy_RepeatMasker...output_log.tabular)  would generate Inerstion and deltion of LTR_counts 
